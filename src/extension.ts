...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.debug.registerDebugConfigurationProvider("rdbg", new RdbgInitialConfigurationProvider()),
    );
    context.subscriptions.push(vscode.debug.registerDebugAdapterDescriptorFactory("rdbg", adapterDescriptorFactory));
    context.subscriptions.push(
        vscode.debug.registerDebugAdapterTrackerFactory("rdbg", new RdbgDebugAdapterTrackerFactory(DAPTrackQueue)),
    );
        ...
        const autoAttachConfigP = (c: AttachConfiguration): boolean => {
            if (c.type === "rdbg" && c.request === "attach" && c.autoAttach) {
                ...
                vscode.window.showErrorMessage(
                    ".vscode/rdbg_autoattach.json contains unexpected contents. Please check integrity.",
                );
            }
            return false;
        };
...
class RdbgDebugAdapterTrackerFactory implements vscode.DebugAdapterTrackerFactory {
    constructor(public readonly _emitter: vscode.EventEmitter<any>) {}
...
class RdbgAdapterDescriptorFactory implements DebugAdapterDescriptorFactory, VersionChecker {
        ...
        try {
            switch (manager) {
                ...
                case VersionManager.Shadowenv:
                    await vscode.extensions.getExtension("shopify.vscode-shadowenv")?.activate();
    ...
    async waitUntil(condition: () => boolean, waitMs: number | undefined) {
        ...
        while (true) {
            ...
            if (i > iterations) {
                vscode.window.showErrorMessage(
                    "Couldn't start debug session (wait for " +
                        (Date.now() - startTime) +
                        " ms). Please install debug.gem.",
                );
                return false;
            }
            if (condition()) {
    ...
    useBundler(config: LaunchConfiguration) {
        const useBundlerFlag =
            config.useBundler !== undefined
                ? config.useBundler
                : vscode.workspace.getConfiguration("rdbg").get("useBundler");
